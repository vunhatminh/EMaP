...
#         self.no_training, self.channels, self.rows, self.cols = data.shape
        ...
        self.mapper = umap.UMAP(n_components = self.dim, random_state = random_state).fit(data)
        self.labels = labels
        self.pivots = None
        self.planes = None
    ...
    def get_pivot(self, labels, no_pivots_per_label = 1, shuffle = False):
        ...
        for l in np.unique(labels):
            all_idx = (labels == l).nonzero()

            if shuffle == False:
                idx = all_idx[0][range(no_pivots_per_label)]
            else:
                idx = all_idx[0][random.sample(range(len(all_idx[0])),no_pivots_per_label )]
...
#         else:
        ...
        pivots_index = self.get_pivot(labels, no_pivots_per_label, shuffle)
        ...
        for i in pivots_index:
            ...
            x_sample, y_sample = self.gen_perturbation_base(np.expand_dims(self.data[i], axis=0),
            ...
            raw_perturbs.append(x_sample[y_sample == 1])
            raw_noise = x_sample[y_sample == 1] - x_sample[y_sample == 0]
            plane_noise = np.zeros_like(raw_noise)
            for d in range(Gv.shape[0]):
                proj = np.dot(raw_noise, Gv[d])
                for s in range(plane_noise.shape[0]):
                    plane_noise[s] = plane_noise[s] + proj[s]*Gv[d]        
            ortho_noise = raw_noise - plane_noise
            orthor_noises.append(ortho_noise)
            plane_noises.append(plane_noise)
            
            x_sample_low = self.transform(x_sample[y_sample == 1].copy())
            x_reverse = np.clip(np.dot(x_sample_low, mat_plane), self.data_min, self.data_max)
             
            pivots_perturbs.append(x_reverse)
        self.raw_perturbs = raw_perturbs
    ...
    def get_G(self, x):
        x_sample = self.gen_perturbation_base(np.expand_dims(x, axis=0),
