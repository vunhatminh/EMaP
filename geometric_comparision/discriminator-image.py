...
for run in range(RUNS):
    ...
    tp_g = get_discriminator_performance(gauss_pc[1::2], np.ones(gauss_pc[1::2].shape[0]) , the_rf_gauss)
    tp_p = get_discriminator_performance(plane_pc[1::2], np.ones(plane_pc[1::2].shape[0]) , the_rf_plane)
    tp_o = get_discriminator_performance(ortho_pc[1::2], np.ones(ortho_pc[1::2].shape[0]) , the_rf_ortho)
...
if BASE_RADIUS == 0:
    discriminator_file = 'result/discriminator/' + EXPERIMENT + '_label_' + target_str + '_dim_' + str(DIM) + '_std_' + str(RADIUS) +'_.pickle'
else:
    discriminator_file = 'result/discriminator/' + EXPERIMENT + '_wbase_label_' + target_str + '_dim_' + str(DIM) + '_std_' + str(RADIUS) +'_.pickle'
